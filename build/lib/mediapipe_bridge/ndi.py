...
class CyndilibNdiReceiver:
    ...
    def open(self) -> None:
        ...
        # 对齐官方示例：遍历 Finder 来查找匹配的 Source 对象
        source = None
        requested = self.source_name
        for src in finder:
            if src.name == requested or src.stream_name == requested:
                source = src
                break
        
        if source is None and requested:
            # 如果没找到，尝试模糊匹配或直接取第一个
            source = finder.get_source(requested)

        if source is None:
            finder.close()
            raise RuntimeError(f'NDI source "{requested}" not found in current network list.')
        ...
        if not receiver.is_connected():
            print(f"Warning: NDI source {requested} not connected yet.")
